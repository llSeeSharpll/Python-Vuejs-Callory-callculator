...
sys.path.append('../')
from entities.user import App_User
...
class Auth():
    ...
    def register(self, username, email, password, mobilePhone,gender,weight,height,age) -> DefaultMethodResult:
        """
        ...
        if error is None:
            newimage = App_User(username=username, email=email, password=password,
                           mobilePhone=mobilePhone,gender=gender,weight=weight,height=height,age=age)
